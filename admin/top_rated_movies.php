<?php 
    ...
    {
        $id = $_GET['id'];
        $delete_sql = "DELETE FROM `top_rated_tbl` WHERE `id`='$id'";
        $delete_res = mysqli_query($conn,$delete_sql);

        if(mysqli_affected_rows($conn) > 0)
        {
            $msg = "Top rated movie delete successfull";
        }
        else 
        { 
            $msg2 = "Top rated movie delete failed";
        }
    }
    ...
    if(isset($_POST['submit']))
    {
        ...
        move_uploaded_file($image_tmp_name,"../storage/top_rated/$image");
    
        $sql = "INSERT INTO `top_rated_tbl`(`image`,`name`,`link`) VALUES('$image','$name','$link')";
        $result = mysqli_query($conn,$sql);
        if(mysqli_affected_rows($conn) > 0)
        {
            $msg3 = "Top rated movie added successfully";
        }
        else 
        {
            $msg4 = "Top rated movie added failed";
        }
...
<div class="container">
    <div class="user_list_in">
        ...
        <form action="top_rated_movies.php" method="post" enctype="multipart/form-data">
            <div class="add_form">
                <div class="row">
                ...
                <?php 
                    ...
                    if(mysqli_num_rows($res) > 0)
                    {
                        ...
                        while($user_row = mysqli_fetch_assoc($res))
                        {
                            echo " <tr>
                                <td>".$sl."</td>
                                <td><img src='../storage/top_rated/".$user_row['image']."' width='70px'></td>
                                <td>".$user_row['name']."</td>
                                <td>".$user_row['link']."</td>
